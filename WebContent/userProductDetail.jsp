									...
									<div class="tab-single">
										<div class="row">
											...
											<div class="col-xl-5 col-lg-4 col-md-4 col-12">
												...
												<%
													try{
														boolean wflag = WishlistDao.checkWishlist(u.getId(), p.getPid());
														if(wflag)
														{
												%>
															<a href="removeFromWishlist.jsp?pid=<%=p.getPid() %>&uid=<%=u.getId() %>">
																<input type="submit" name="action" value="Remove From Wishlist" class="btn btn-primary">
															</a>
												<%
														}
														else
														{
												%>
															<a href="addToWishlist.jsp?pid=<%=p.getPid() %>&uid=<%=u.getId() %>">
																<input type="submit" name="action" value="Add To Wishlist" class="btn btn-primary">
															</a>
												<%
														}
														boolean cflag = CartDao.checkCart(u.getId(), p.getPid());
														if(cflag)
														{
												%>
															<a href="removeFromCart.jsp?pid=<%=p.getPid() %>&uid=<%=u.getId() %>">
																<input type="submit" name="action" value="Remove From Cart" class="btn btn-primary">
															</a>
												<%
														}
														else
														{
												%>
															<a href="addToCart.jsp?pid=<%=p.getPid() %>&uid=<%=u.getId() %>">
